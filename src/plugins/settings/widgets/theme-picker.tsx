        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <For each={themes()}>
            {(theme) => (
              ...
              >
                <div class={`relative rounded-lg border-2 overflow-hidden transition-all ${
                  props.selectedThemeId === theme.id ? 'border-accent ring-2 ring-accent/50' : 'border-border-primary group-hover:border-accent'
                }`}>
                  <div class="h-32" style={{ 'background-color': theme.colors.bgPrimary }}>
                    <div class="p-3">
                      <div class="h-4 w-1/3 rounded-sm" style={{ 'background-color': theme.colors.accent }}></div>
                      <div class="mt-2 space-y-1">
                        <div class="h-2 w-4/5 rounded-full" style={{ 'background-color': theme.colors.textPrimary }}></div>
                        <div class="h-2 w-3/5 rounded-full" style={{ 'background-color': theme.colors.textSecondary }}></div>
                      </div>
                      <div class="absolute bottom-3 right-3 flex space-x-1">
                        <div class="w-5 h-5 rounded-full" style={{ 'background-color': theme.colors.primary }}></div>
                        <div class="w-5 h-5 rounded-full" style={{ 'background-color': theme.colors.accent }}></div>
                        <div class="w-5 h-5 rounded-full" style={{ 'background-color': theme.colors.bgSecondary }}></div>
                      </div>
                    </div>
                  </div>
                  <Show when={props.selectedThemeId === theme.id}>
                    <div class="absolute inset-0 bg-accent/10"></div>
                    <div class="absolute top-2 right-2 bg-accent text-text-inverse rounded-full p-1">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                    </div>
                  </Show>
                </div>
                <div class="mt-2 text-center">
                    <h4 class="font-medium text-sm">{theme.displayName}</h4>
